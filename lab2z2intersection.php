...
$response_html = "<table  class=\"table table-bordered\">
	<thead>
	<tr>
		<th>i</th>
		<th>j</th>
...
function merge_intersection ($a, $b, $i = 0, $j = 0){
	global $SumArray;
	...
	if (count($a) > $i){
		if ($a[$i] < $b[$j]) {
			merge_intersection($a, $b, $i+1, $j);
		} elseif ($a[$i] > $b[$j]) {
			if (count($b) > $j)
				merge_intersection($a, $b, $i, $j+1);
		} elseif ($a[$i] == $b[$j]) {
			array_push($SumArray, $a[$i]);
			merge_intersection($a, $b, $i+1, $j+1);
		}
	}
	//Проходимо обидва масиви методом злиття
}

merge_intersection($nb, $na);
